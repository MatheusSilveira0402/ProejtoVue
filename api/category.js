    ...
    const  { existsOrError, notExistsOrError, equalsOrError} = app.api.validation
        ...
        const save = (req, res) => {
            ...
            if(category.id){
                app.db('categories')
                    ...
                    .catch( err => {res.status(500).send(err), console.log(err, 'erro na função de realizar update da categoria')})
            } else {
                app.db('categories')
                    .insert(category)
                    .then(() => res.status(204).send())
                    .catch(err => {res.status(500).send(err), console.log(err, 'erro na função de inserir a categoria')})
            }
        }
        const remove = async (req, res) => {
            try {
                ...
                res.status(400).send(msg), console.log(msg)
            }
        }
